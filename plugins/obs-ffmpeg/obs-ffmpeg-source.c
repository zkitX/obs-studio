...
static obs_properties_t *ffmpeg_source_getproperties(void *data)
{
	...
	obs_property_list_add_string(prop,
			obs_module_text("BehaviorStopRestart"),
			"stop_restart");
	obs_property_list_add_string(prop,
...
static void dump_source_info(struct ffmpeg_source *s, const char *input)
{
	const char *behaviors[] =
			{"Stop/Restart", "Pause/Unpause", "Always Play"};
		...
		obs_hotkey_t *hotkey, bool pressed)
{
	...
	if (pressed && obs_source_active(s->source))
		ffmpeg_source_play_pause(s, true);
}
...
static void *ffmpeg_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	s->play_pause_hotkey = obs_hotkey_pair_register_source(
			s->source,
			"MediaSource.Play", obs_module_text("Play"),
			"MediaSource.Pause", obs_module_text("Pause"),
			ffmpeg_source_play_hotkey, ffmpeg_source_pause_hotkey,
			s, s);
	...
	s->stop_hotkey = obs_hotkey_register_source(source,
			"MediaSource.Stop",
			obs_module_text("Stop"),
			ffmpeg_source_stop_hotkey, s);

	s->playlist_next_hotkey = obs_hotkey_register_source(source,
			"MediaSource.PlaylistNext",
			obs_module_text("PlaylistNext"),
			ffmpeg_source_playlist_next_hotkey, s);

	s->playlist_prev_hotkey = obs_hotkey_register_source(source,
			"MediaSource.PlaylistPrev",
